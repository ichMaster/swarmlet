...
Use the phase label derived from the phase number argument to fetch issues:

```!
PHASE_NUM=$(echo "$ARGUMENTS" | grep -oE '^[0-9]+')
LABEL_MAP="1:phase:1-foundation 2:phase:2-lexer-parser 3:phase:3-builtins 4:phase:4-evaluator 5:phase:5-engine 6:phase:6-frontend 7:phase:7-examples 8:phase:8-quality 9:phase:9-docs 11:phase:11-education"
LABEL=$(echo "$LABEL_MAP" | tr ' ' '\n' | grep "^${PHASE_NUM}:" | cut -d: -f2-)
if [ -n "$LABEL" ]; then
  gh issue list --repo ichMaster/swarmlet --label "$LABEL" --state open --json number,title,body --limit 50 2>/dev/null
else
  echo "Unknown phase number: $PHASE_NUM"
fi
```
